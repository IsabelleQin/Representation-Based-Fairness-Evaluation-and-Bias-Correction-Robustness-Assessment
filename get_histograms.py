...
for dataset in ['adult', 'bank', 'german']:
    ...
    for model_file in model_files:
        ...
        for label in ['pos', 'neg']:
            ...
            for subgroup in ['full']+['s=%d'%j for j in sens_choices]:
                hist_dict = {header:[] for header in ['layerId', 'nodeId', 'binId', 'sigmaInterval_lb', 'sigmaInterval_ub', 'sigmaFreq']}
                if not os.path.exists('./hist_subgroups/%s/%s/%s/%s_%s_hist.csv'%(dataset, sens_att, subgroup, model_name, label)):
                    for node_id in np.unique(contrib_dict_train['nodeId']):
                        # Filter contrib of given node
                        if label == 'neg':
                            if subgroup == 'full':
                                contribs_node = [contrib for i, contrib in enumerate(contrib_dict_train['nodeContrib']) if (contrib_dict_train['nodeId'][i] == node_id and contrib_dict_train['inputId'][i] in neg_idx)]
                            else:
                                contribs_node = [contrib for i, contrib in enumerate(contrib_dict_train['nodeContrib']) if (contrib_dict_train['nodeId'][i] == node_id and contrib_dict_train['inputId'][i] in neg_idx and contrib_dict_train['inputId'][i] in subgroup_idx[subgroup])]
                        else:
                            if subgroup == 'full':
                                contribs_node = [contrib for i, contrib in enumerate(contrib_dict_train['nodeContrib']) if (contrib_dict_train['nodeId'][i] == node_id and contrib_dict_train['inputId'][i] in pos_idx)]
                            else:
                                contribs_node = [contrib for i, contrib in enumerate(contrib_dict_train['nodeContrib']) if (contrib_dict_train['nodeId'][i] == node_id and contrib_dict_train['inputId'][i] in pos_idx and contrib_dict_train['inputId'][i] in subgroup_idx[subgroup])]
                    ...
                    pd.DataFrame(hist_dict).to_csv('./hist_subgroups/%s/%s/%s/%s_%s_hist.csv'%(dataset, sens_att, subgroup, model_name, label), index=None)
